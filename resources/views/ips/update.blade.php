...
<x-app-layout>
    <div class="max-w-2xl mx-auto px-4 py-8 bg-white shadow rounded-lg">
        ...
        <form action="{{ route('ips.update', $ip) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            @csrf
            @method('PATCH')
            ...
            <div>
                ...
                <select name="employer_id" id="employer_id" required
                    ...
                    @foreach($employers as $employer)
                        <option value="{{ $employer->id }}" {{ old('employer_id', $ip->employer_id) == $employer->id ? 'selected' : '' }}>
                        {{ $employer->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700">Perspective</label>
                <input type="text" name="perspective" value="{{ old('perspective', $ip->perspective) }}" required
            ...
            <div>
                ...
                <input type="text" name="responsible_unit" value="{{ old('responsible_unit', $ip->responsible_unit) }}" required
                    class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
            ...
            <div>
                ...
                <input type="text" name="specific_task" value="{{ old('specific_task', $ip->specific_task) }}" required
                    class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                @error('specific_task')
                    <div class="text-sm text-red-600 mt-1">{{ $message }}</div>
                @enderror
            ...
            <div>
                ...
                <input type="text"  name="requestor" value="{{ old('requestor', $ip->requestor) }}" required
                    class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                @error('requestor')
                    <div class="text-sm text-red-600 mt-1">{{ $message }}</div>
                @enderror
            ...
            <div>
                ...
                <input type="text" name="target_output" value="{{ old('target_output', $ip->target_output) }}" required
                    class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                @error('target_output')
                    <div class="text-sm text-red-600 mt-1">{{ $message }}</div>
                @enderror
            ...
            <div>
                ...
                <input type="text" name="actual_output" value="{{ old('actual_output', $ip->actual_output) }}" 
                    class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                @error('actual_output')
                    <div class="text-sm text-red-600 mt-1">{{ $message }}</div>
                @enderror
            ...
            <div>
                ...
                <select name="status" id="status" required
                    ...
                    <option value="Pending" {{ old('status', $ip->status) == 'Pending' ? 'selected' : '' }}>Pending</option>
                    <option value="Completed" {{ old('status', $ip->status) == 'Completed' ? 'selected' : '' }}>Completed</option>
                    <option value="Transmitted" {{ old('status', $ip->status) == 'Transmitted' ? 'selected' : '' }}>Transmitted</option>
                </select>
                @error('status')
                    <div class="text-sm text-red-600 mt-1">{{ $message }}</div>
                @enderror
            ...
            <div>
                ...
                <input type="date" name="status_date" value="{{ old('status_date', $ip->status_date) }}" required
                    class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                @error('status_date')
                    <div class="text-sm text-red-600 mt-1">{{ $message }}</div>
                @enderror
            ...
            <div>
                ...
                <textarea name="remarks" rows="4"
                    class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                    placeholder="Enter remarks or notes here...">{{ old('remarks', $ip->remarks) }}</textarea>
                @error('remarks')
                    <div class="text-sm text-red-600 mt-1">{{ $message }}</div>
                @enderror
            </div>
